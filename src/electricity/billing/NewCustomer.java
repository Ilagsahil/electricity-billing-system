...
public class NewCustomer extends JFrame implements ActionListener{
    ...
    if (ae.getSource() == next) {
        ...
        Matcher emailMatcherResult = emailMatcher.matcher(email);

        // Check if the password length is greater than 7
         // You need to get the password from the user input field
        if (phone.length() <= 7) {
            JOptionPane.showMessageDialog(null, "Password must be at least 8 characters long", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        if (!emailMatcherResult.matches()) {
            JOptionPane.showMessageDialog(null, "Invalid Email Address", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        // The rest of your code for database insertion
